                ...
                switch( intst & ( USBFS_UIS_TOKEN_MASK | USBFS_UIS_ENDP_MASK ) )
                {
                            ...
                            if( intst & USBFS_UIS_TOG_OK )
                            {
                                if ( ( USBFS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
                                {
                                    if (( USBFS_SetupReqType & USB_REQ_TYP_MASK ) == USB_REQ_TYP_CLASS)
                                    {
                                        ...
                                        {
                                            case HID_SET_REPORT:
                                                memcpy(&HID_Report_EP0_Buffer[0],USBFS_EP0_Buf,DEF_USBD_UEP0_SIZE);
                    ...
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP1:
                        if ( intst & USBFS_UIS_TOG_OK )
                        {
                            ...
                            // !!!!
                            if (usb_callbk_pFun != NULL)
                            {
                                for(int a = 0;a < RingBuffer_Comm.PackLen[RingBuffer_Comm.LoadPtr];a++)
                                {
                        ...
                        switch( USBFS_SetupReqCode )
                        {
                            ...
                            case HID_GET_REPORT:
                                if( USBFS_SetupReqIndex == 0x00 )
                                {
                                    ...
                                    memcpy(USBFS_EP0_Buf,&HID_Report_EP0_Buffer[0],DEF_USBD_UEP0_SIZE);
                                }
                                else
                                {
                                    errflag = 0xFF;
