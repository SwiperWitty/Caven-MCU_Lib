...
static int ADC_Start_Init(int Set)
{
    ...
    #if (Exist_ADC & OPEN_0001)
    Channel_NUMs += 2;
    adc_ordinary_channel_set(ADC1, ADC_IO_PA0, Channel_NUMs - 1, ADC_Speed);
    adc_ordinary_channel_set(ADC1, ADC_IO_PA1, Channel_NUMs, ADC_Speed);
    #endif
    #if (Exist_ADC & OPEN_0010)
    Channel_NUMs += 2;
    adc_ordinary_channel_set(ADC1, ADC_IO_PA2, Channel_NUMs - 1, ADC_Speed);
    adc_ordinary_channel_set(ADC1, ADC_IO_PA3, Channel_NUMs, ADC_Speed);
    #endif
    #if (Exist_ADC & OPEN_0100)
    Channel_NUMs += 2;
    adc_ordinary_channel_set(ADC1, ADC_IO_PA4, Channel_NUMs - 1, ADC_Speed);
    adc_ordinary_channel_set(ADC1, ADC_IO_PA5, Channel_NUMs, ADC_Speed);
    #endif
    #if (Exist_ADC & OPEN_1000)
    Channel_NUMs += 2;
    adc_ordinary_channel_set(ADC1, ADC_IO_PA6, Channel_NUMs - 1, ADC_Speed);
    ...
    adc_base_struct.repeat_mode = TRUE;                         // 是否需要重复
    adc_base_struct.data_align = ADC_RIGHT_ALIGNMENT;
    adc_base_struct.ordinary_channel_length = Channel_NUMs;     // Channel_NUMs
...
float ADC_Get_Temperature_Fun(void)
{
    ...
    Temp = ADC1_DMA_list[Channel_NUMs-1];                                     //温传是最后一个
