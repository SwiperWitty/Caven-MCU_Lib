...
extern uint8_t HIDTxBuffer[],HIDRxBuffer[],HIDKey[];      // dd-dd

extern uint8_t Endp1Busy,Endp2Busy;         // dd-dd
...
ONE_DESCRIPTOR Report_Descriptor[2] =         // dd-dd
{
    {(uint8_t*)USBD_KeyRepDesc,KEYBOARD_SIZE_HID_DESC},         //Ë³Ðò²»ÄÜ´í!!!
...
void USBD_Reset(void)
{
  ...
  _ClearDTOG_TX(ENDP0);
    ...
    SetEPType(ENDP1, EP_INTERRUPT);       // dd-dd
    SetEPTxAddr(ENDP1, ENDP1_TXADDR);
    SetEPRxAddr(ENDP1, ENDP1_RXADDR);
    SetEPRxCount(ENDP1, ENDP1_IN_SIZE);
 ...
 */
uint8_t *USBD_GetStringDescriptor(uint16_t Length)          // ÅäÖÃÃèÊö·ûºÏ¼¯
{
 ...
 * @return  The address of the device descriptor.
 */
uint8_t *USBD_GetReportDescriptor(uint16_t Length)          //±¨¸æÃèÊö·û
{
...
void USBD_HID_Keyboard_Updata(void)
{
    while (Endp1Busy);
    UserToPMABufferCopy(HIDKey, ENDP1_TXADDR, ENDP1_IN_SIZE);
    SetEPTxCount(ENDP1, ENDP1_IN_SIZE);
    SetEPTxValid(ENDP1);
    Endp1Busy = TRUE;
}
__attribute__((optimize("O0")))
void USBD_HID_Data_Updata(void)          // dd-dd
{
    while (Endp2Busy);
