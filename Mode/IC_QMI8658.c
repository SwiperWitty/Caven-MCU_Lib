...
uint8_t qmi8658_write_reg(unsigned char reg, unsigned char value)
{
	uint8_t ret = 0;
    uint8_t write_buf[2];
    write_buf[0] = reg;
    write_buf[1] = value;
	Base_IIC_Send_DATA(QMI8658_SLAVE_ADDR,write_buf,1,sizeof(write_buf),10,0);
	return ret;
}
...
{
    uint8_t retval = 0;
	retval = Base_IIC_Send_DATA(QMI8658_SLAVE_ADDR,&reg,1,sizeof(reg),10,1);
	retval |= Base_IIC_Receive_DATA(QMI8658_SLAVE_ADDR,buf,1,len,10);
	return retval;
}
...
unsigned char qmi8658_get_id(void)
{
    ...
    g_imu.slave = QMI8658_SLAVE_ADDR;
    retry = 0;
